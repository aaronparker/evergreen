...
Describe -Tag "Get" -Name "Get-EvergreenApp works with supported application: <application>" -ForEach $Applications {
    ...
    Context "Validate Get-EvergreenApp works with: <application>." -ForEach $Output {
        ...
        It "Get-EvergreenApp -Name <application> should return a count of 1 or more" -ForEach $Item {
            ($_ | Measure-Object).Count | Should -BeGreaterThan 0
        }

        # Test that the output has a Version property and that property is a string
        It "Output for <application> should have a Version property that is a string" -ForEach $Item {
            $_.Version | Should -BeOfType [System.String]
        }

        # Test that output with Version property is valid
        It "Output for <application> should have a valid version number" -ForEach $Item {
            $_.Version | Should -Match $MatchVersions
        }

        # Test that the output has a URI property and that property is a string
        It "Output for <application> should have a URI property that is a string" -ForEach $Item {
