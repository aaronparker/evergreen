function Get-mySQLConnectorNET {
    ...
    if ($Null -ne $object) {
        foreach ($Architecture in $res.Get.Download.Uri.GetEnumerator()) {
            $Uri = $res.Get.Download.Uri[$Architecture.Key] -replace $res.Get.Download.ReplaceVersionShort, (($object.Version -split '\.')[0,1] -join '.') -replace $res.Get.Download.ReplaceVersion, $object.Version
